...
{
    var invalidBlue = game.Input.Any(x => x.Blue > 14);
    var invalidRed = game.Input.Any(x => x.Red > 12);
    var invalidGreen = game.Input.Any(x => x.Green > 13);

    if (!invalidBlue && !invalidGreen && !invalidRed)
    {
        sum += game.Number;
    }
}
...
foreach (var game in games)
{
    var minBlues = game.Input.MaxBy(i=>i.Blue)!.Blue;
    var minReds = game.Input.MaxBy(x => x.Red)!.Red;
    var minGreens = game.Input.MaxBy(x => x.Green)!.Green;

    var gameInputPower = minBlues * minReds * minGreens;
...
";
    ...
    Game[] gameInput = input.Split("\n").Select(x => x.Trim()).Where(x => !string.IsNullOrEmpty(x))
        .Select(x => x.Replace("Game ", ""))
        .Select(x => x.Split(':'))
        .Select(x =>
        {
            var gameNumber = int.Parse(x[0]);
            var rawGameInputs = x[1].Split(';');
            List<GameInput> gameInputs = new List<GameInput>();

            foreach (var item in rawGameInputs)
            {
                int blue = 0;
                int red = 0;
                int green = 0;

                foreach (var sample in item.Split(','))
                {
                    if (sample.Contains("blue"))
                    {
                        blue = int.Parse(sample.Replace("blue", "").Trim());
                    }
                    if (sample.Contains("red"))
                    {
                        red = int.Parse(sample.Replace("red", "").Trim());
                    }
                    if (sample.Contains("green"))
                    {
                        green = int.Parse(sample.Replace("green", "").Trim());
                    }
                }

                gameInputs.Add(new GameInput(blue, red, green));
            }

            return new Game(gameNumber, gameInputs);
...
public record Game(int Number, IEnumerable<GameInput> Input);
public record GameInput(int Blue, int Red, int Green);