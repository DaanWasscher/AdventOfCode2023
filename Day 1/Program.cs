...
foreach (var inputLine in inputs)
{
    ...
    if (match.Success)
    {
        ...
        sum += long.Parse(string.Concat(first, second));
    }
    else
    {
...
// part 2
var regexPartTwo = new Regex("(\\d|one|two|three|four|five|six|seven|eight|nine)");
var regexPartTwoRtl = new Regex("(\\d|one|two|three|four|five|six|seven|eight|nine)", RegexOptions.RightToLeft);
long sumPartTwo = 0;

foreach (var inputLine in inputs)
{
    var matches = regexPartTwo.Matches(inputLine);
    var matchesRtl = regexPartTwoRtl.Matches(inputLine);

    var first = matches[0].Value;
        var second = matchesRtl[0].Value;

        first = decodeStringToDigit(first);
        second = decodeStringToDigit(second);

        sumPartTwo += long.Parse(string.Concat(first, second));
}

Console.WriteLine(sumPartTwo);

static string decodeStringToDigit(string input)
{
    switch (input)
    {
        case "one":
