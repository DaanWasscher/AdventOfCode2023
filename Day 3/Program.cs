...
List<NumberWithStar> numbersWithStarPosition = new List<NumberWithStar>();
...
for (int lineIndex = 0; lineIndex < inputLines.Length; lineIndex++)
{
    ...
    for (int i = 0; i < line.Length; i++)
    {
        ...
        else
        {
            if (startPosition != -1)
            {
                var specialChar = hasSpecialNearby(inputLines, lineIndex, startPosition, number.Length);
                if (specialChar is not null)
                {
                    // sum any number that has a special char nearby
                    sum += int.Parse(number);
                }
                if (specialChar is not null && specialChar.IsStarChar)
                {
                    // track numbers with a star position for challenge 2
                    numbersWithStarPosition.Add(new NumberWithStar(specialChar, int.Parse(number)));
                }
            }
            startPosition = -1;
            number = "";
...
var lookup = numbersWithStarPosition.ToLookup(s => s.Location, s => s.Number);
sum2 = lookup.Where(l => l.Count() == 2).Select(l => l.ElementAt(0) * l.ElementAt(1)).Sum();


Console.WriteLine(sum2);


SpecialCharLocation? hasSpecialNearby(string[] inputLines, int lineIndex, int startPosition, int length)
{
    SpecialCharLocation? hasSpecialNearby = null;
    //check previous line
    if (lineIndex != 0)
    {
        hasSpecialNearby = hasSpecialOnLineAtPositionRange(inputLines, lineIndex - 1, startPosition, length);
    }

    //check current line
    if (hasSpecialNearby is null || !hasSpecialNearby.IsStarChar)
    {
        var hasSpecialNearbyTemp = hasSpecialOnLineAtPositionRange(inputLines, lineIndex, startPosition, length);
        hasSpecialNearby ??= hasSpecialNearbyTemp;
        if (hasSpecialNearby is not null && hasSpecialNearbyTemp is not null && hasSpecialNearbyTemp.IsStarChar)
        {
            hasSpecialNearby = hasSpecialNearbyTemp;
        }
    }
    ...
    //check next line
    if (lineIndex != 139 && (hasSpecialNearby is null || !hasSpecialNearby.IsStarChar))
    {
        var hasSpecialNearbyTemp = hasSpecialOnLineAtPositionRange(inputLines, lineIndex + 1, startPosition, length);
        hasSpecialNearby ??= hasSpecialNearbyTemp;
        if (hasSpecialNearby is not null && hasSpecialNearbyTemp is not null && hasSpecialNearbyTemp.IsStarChar)
        {
            hasSpecialNearby = hasSpecialNearbyTemp;
    ...
    return hasSpecialNearby;
}

SpecialCharLocation? hasSpecialOnLineAtPositionRange(string[] inputLines, int lineIndex, int startPosition, int length)
{
    ...
    if (startPosition != 0)
    {
        ...
        adjustedLength = length + 2;
    }

    var overflow = adjustedStartPosition + 1 + adjustedLength - 140;
    if (overflow > 0)
    {
        adjustedLength -= overflow;
    }

    var searchString = inputLines[lineIndex].Substring(adjustedStartPosition, adjustedLength);
    if (searchString.Any(c => c != '.' && !char.IsDigit(c)))
    {
        var starPosition = searchString.IndexOf('*');
        return new SpecialCharLocation(starPosition != -1, lineIndex, starPosition + adjustedStartPosition);
    }
...
public record SpecialCharLocation(bool IsStarChar, int lineIndex, int startPosition);

public record NumberWithStar(SpecialCharLocation Location, int Number);